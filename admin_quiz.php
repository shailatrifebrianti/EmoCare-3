...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (($_POST['action'] ?? '') === 'create_quiz')) {
  ...
  if ($question === '' || count($valid_opts) < 2) {
    ...
    try {
      ...
      $mysqli->commit();
      header('Location: admin_quiz.php?cat=' . urlencode($cat) . '&saved=1#quiz');
      exit;
    } catch (Throwable $e) {
      $mysqli->rollback();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (($_POST['action'] ?? '') === 'toggle_active')) {
  ...
  if ($qid > 0) {
    ...
    $stmt->bind_param('is', $qid, $cat);
    $stmt->execute();
    $stmt->close();
    header('Location: admin_quiz.php?cat=' . urlencode($cat) . '&updated=1#quiz');
    exit;
  }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (($_POST['action'] ?? '') === 'delete_quiz')) {
  ...
  if ($qid > 0) {
    ...
    try {
      ...
      $d2->bind_param('is', $qid, $cat);
      $d2->execute();
      $d2->close();
      $mysqli->commit();
      header('Location: admin_quiz.php?cat=' . urlencode($cat) . '&deleted=1#quiz');
      exit;
    } catch (Throwable $e) {
      $mysqli->rollback();
      $flash = 'Gagal menghapus: ' . $e->getMessage();
    }
  }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && (($_POST['action'] ?? '') === 'update_quiz')) {
  ...
  if ($qid <= 0 || $qtext === '' || count(array_filter($opts)) < 2) {
    ...
    try {
      ...
      $u->bind_param('sis', $qtext, $qid, $cat);
      $u->execute();
      $u->close();
      ...
      $mysqli->rollback();
      $flash = 'Gagal update: ' . $e->getMessage();
    }
  }
}
...
$q->execute();
if ($r = $q->get_result())
  $quizzes = $r->fetch_all(MYSQLI_ASSOC);
$q->close();
?>
<!doctype html>
<html lang="id">
...
<body data-page="admin">
  <div class="wrap">
    ...
    <?php if (!empty($_GET['saved'])): ?>
      <div class="ok">Kuis tersimpan.</div><?php endif; ?>
    <?php if (!empty($_GET['updated'])): ?>
      <div class="ok">Data diperbarui.</div><?php endif; ?>
    <?php if (!empty($_GET['deleted'])): ?>
      <div class="ok">Kuis dihapus.</div><?php endif; ?>
    <?php if ($flash): ?>
      <div class="warn"><?= h($flash) ?></div><?php endif; ?>

    <section class="card full" id="quiz">
      <div class="hd">
        <h2>Buat Kuis • <?= h($CAT_LABEL[$cat]) ?></h2>
      </div>
      <div class="bd">
          ...
          <div class="row row-2">
            <label>
              <div class="muted small">Opsi 1</div><input class="inp" name="option_1" required
                placeholder="Mis: Tidak Pernah">
            </label>
            <label>
              <div class="muted small">Opsi 2</div><input class="inp" name="option_2" required
                placeholder="Mis: Jarang">
            </label>
            <label>
              <div class="muted small">Opsi 3</div><input class="inp" name="option_3" placeholder="Mis: Sering">
            </label>
            <label>
              <div class="muted small">Opsi 4</div><input class="inp" name="option_4" placeholder="Mis: Sangat Sering">
            </label>
          </div>
    ...
    <section class="card full">
      <div class="hd">
        <h2>Kuis Terbaru • <?= h($CAT_LABEL[$cat]) ?></h2>
      </div>
              ...
              <?php foreach ($quizzes as $q): ?>
                <tr>
                  ...
                  <td class="actions">
                    ...
                    <a class="btn ghost"
                      href="admin_quiz.php?cat=<?= urlencode($cat) ?>&edit_id=<?= (int) $q['id'] ?>#quiz">Edit</a>

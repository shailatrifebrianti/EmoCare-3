...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_quiz') {
  ...
  if ($qid > 0) {
    ...
    try {
      ...
      $delOpt->bind_param('i', $qid); $delOpt->execute(); $delOpt->close();
      $delQ = $mysqli->prepare("DELETE FROM quiz_questions WHERE id=?");
      $delQ->bind_param('i', $qid); $delQ->execute(); $delQ->close();
      $mysqli->commit();
      header('Location: admin.php?deleted=1#quiz'); exit;
    } catch (Throwable $e) {
      $mysqli->rollback();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update_quiz') {
  ...
  if ($qid <= 0 || $qtext === '' || count(array_filter($opts)) < 2) {
    ...
    try {
      ...
      $mysqli->commit();
      header('Location: admin.php?updated=1#quiz'); exit;
    } catch (Throwable $e) {
      $mysqli->rollback();
...
<head>
  ...
  <style>
    ...
    .qtext{max-width:70ch;overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;}
  </style>
</head>
<body data-page="admin">
  <div class="wrap">
    ...
    <?php if (!empty($_GET['saved'])): ?><div class="ok">Kuis berhasil disimpan.</div><?php endif; ?>
    <?php if (!empty($_GET['updated'])): ?><div class="ok">Data berhasil diperbarui.</div><?php endif; ?>
    <?php if (!empty($_GET['deleted'])): ?><div class="ok">Data berhasil dihapus.</div><?php endif; ?>
    <?php if ($flash): ?><div class="warn"><?= h($flash) ?></div><?php endif; ?>

    <!-- BUAT KUIS -->
    <section class="card full" id="quiz">
      <div class="hd"><h2>Buat Kuis</h2></div>
      <div class="bd">
        <form method="post" class="row">
          ...
          <div class="row row-2">
            <label><div class="muted small">Opsi 1</div><input class="inp" name="option_1" required placeholder="Tidak Pernah" /></label>
            <label><div class="muted small">Opsi 2</div><input class="inp" name="option_2" required placeholder="Jarang" /></label>
            <label><div class="muted small">Opsi 3</div><input class="inp" name="option_3" placeholder="Sering" /></label>
            <label><div class="muted small">Opsi 4</div><input class="inp" name="option_4" placeholder="Sangat Sering" /></label>
          </div>
          ...
          <form method="post" class="row">
            ...
            <label>
              ...
              <select class="inp" name="edit_category" required>
                <option value="self_esteem"     <?= ($edit['category']??'')==='self_esteem'?'selected':'' ?>>Self-Esteem</option>
                <option value="social_anxiety"  <?= ($edit['category']??'')==='social_anxiety'?'selected':'' ?>>Kecemasan Sosial</option>
              </select>
            </label>
    ...
    <section class="card full">
      <div class="hd"><h2>Kuis Terbaru</h2></div>
      <div class="bd table-responsive">
        <table class="ec-table">
          ...
          <tbody>
            ...
            <tr>
              <td><?= (int)$q['id'] ?></td>
              <td><div class="qtext"><?= h($q['question_text']) ?></div></td>
    ...
    <!-- DAFTAR ADMIN -->
    <section class="card full">
      <div class="hd"><h2>Daftar Akun Admin</h2></div>
      <div class="bd table-responsive">
        <?php if (empty($admins)): ?>
    ...
    <!-- DAFTAR USER -->
    <section class="card full">
      <div class="hd"><h2>Daftar Akun User</h2></div>
      <div class="bd table-responsive">
        <?php if (empty($users)): ?>

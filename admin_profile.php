...
function is_admin(mysqli $db): bool
{
  ...
  if ($uid <= 0)
    return false;
  $st = $db->prepare("SELECT role FROM pengguna WHERE pengguna_id=?");
  $st->bind_param('i', $uid);
  $st->execute();
  $role = $st->get_result()->fetch_column();
  $st->close();
  return $role === 'admin';
}
if (!is_admin($mysqli)) {
...
<head>
  ...
  <style>
    ...
    .side-hd {
      ...
      background: linear-gradient(180deg, var(--pink-200), #fff)
    }
      ...
      <div class="topbar">
        ...
        <div class="right">
          ...
          <form action="backend/auth_logout.php" method="post" style="margin:0"><button
              class="btn ghost">Keluar</button></form>
        </div>
      </div>
        ...
        <div class="panel" id="p1" role="tabpanel" aria-labelledby="t1">
          <div class="grid-2">
            <div class="section">
              <div style="display:flex;justify-content:space-between;align-items:center;gap:8px">
                ...
                <button class="btn ghost" type="button" onclick="document.getElementById('about').focus()">Ganti Tentang
                  Saya</button>
              </div>
          ...
          <div class="section" style="margin-top:12px">
            ...
            <div class="kv">
              ...
              <div class="v"><strong><?= h($me['nama']) ?></strong></div>
              <div class="k">Email</div>
              <div class="v"><?= h($me['email']) ?></div>
              <div class="k">Role</div>
              <div class="v"><span class="pill"><?= h($me['role']) ?></span></div>
              <div class="k">Dibuat</div>
              <div class="v"><?= h($me['created_at'] ?: '-') ?></div>
            </div>
          </div>
        </div>

...
$res = $stmt->get_result();
$ids = [];
while ($row = $res->fetch_assoc()) { $qs[] = $row; $ids[] = (int)$row['id']; }
$stmt->close();
...
foreach ($qs as $q) {
  ...
  if (count($opts) < 2) $opts = ['Tidak Pernah','Jarang','Sering','Sangat Sering'];
...
<head>
  ...
  <style>
    .panel{max-width:760px;margin:24px auto;padding:16px;border-radius:18px;background:#fff;
    ...
    .result{background:linear-gradient(180deg,#fff0f6,#fff);border:1px solid #f6c3d3;border-radius:16px;padding:14px;margin-top:12px}
    .score{font-size:28px;font-weight:800}
    .tag{padding:4px 10px;border:1px solid #f1b9cd;border-radius:999px;font-weight:600}
  </style>
</head>
<body>
  <div class="panel">
